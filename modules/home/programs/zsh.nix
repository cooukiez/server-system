...
}:
{
  programs.zsh = {
    ...
    shellAliases = {
      ...
      nus = "sync-flake && nh os switch /etc/nixos#nixosConfigurations.${hostname} --update";
      nuh = "sync-flake && nh home switch /etc/nixos#homeConfigurations.${userConfig.name}@${hostname}.activationPackage";
      nuuh = "sync-flake && nh home switch /etc/nixos#homeConfigurations.${userConfig.name}@${hostname}.activationPackage --update";
